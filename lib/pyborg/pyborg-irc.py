...
!stealth, !unignore, !wakeup, !talk, !me, !owner"
    ...
    def _failed_new_nickname(self, c, e):
        if self.attempting_regain is False:
            self.settings.myname = c.get_nickname()[:8] + repr(random.randint(0, 9))
...
#               self.channels = IRCDict()
    ...
    def on_msg(self, c, e):
        ...
        if e.eventtype() == "pubmsg":
            escaped_users = list(map(re.escape, self.channels[target].users()))
        ...
        if source in self.owners and e.source() in self.owner_mask:
            ...
            #start a new thread
            _thread.start_new_thread(self.pyborg.process_msg, (self, body, replyrate, learn, (body, source, target, c, e)))
    ...
    def irc_commands(self, body, source, target, c, e):
        ...
        if command_list[0] == "!replyrate" and len(command_list)==1:
            msg = "Reply rate is "+repr(self.settings.reply_chance)+"%."
    ...
    def output(self, message, args):
        ...
        # Private messages
            ...
            # ctcp action priv msg
                ...
                # send copy to owner
                if not source in self.owners:
                    list(map ((lambda x: c.action(x, "(From "+source+") "+body)), self.owners))
                    list(map ((lambda x: c.action(x, "(To   "+source+") "+message)), self.owners))
...
if __name__ == "__main__":
    ...
    try:
        ...
        traceback.print_exc()
        c = input("Ooops! It looks like Pyborg has crashed. Would you like to save its dictionary? (y/n) ")
        if c.lower()[:1] == 'n':
            sys.exit(0)
    bot.autosave_stop()
